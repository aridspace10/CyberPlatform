...
def test_sed_error(cl, shell_empty: ShellState):
    ...
    assert CmdResult.stderr == ["sed [OPTION]... {script-only-if-no-other-script} [input-file]..."]
...
def test_sed_addressing2(cl, shell_sed: ShellState):
    ...
    assert CmdResult.stdout == ["cat wolf cat", "hi dog", " whats up", " the dog", " test cat here"]
    assert CmdResult.stderr == []
...
def test_sed_multexpr(cl, shell_sed: ShellState):
    CmdResult = cl.enter_command("sed --expression=s/cat/dog/ -e 's/wolf/howl/' f1.txt", shell_sed)
