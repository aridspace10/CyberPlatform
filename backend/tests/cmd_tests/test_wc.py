...
def test_wc_l_empty_file(cl, shell_emptyfile: ShellState):
    CommandResult = cl.enter_command("wc -l empty.txt", shell_emptyfile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_l_single_line_no_newline(cl, shell_one_line_no_newline: ShellState):
    CommandResult = cl.enter_command("wc -l f1.txt", shell_one_line_no_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_l_single_line_with_newline(cl, shell_one_line_newline: ShellState):
    CommandResult = cl.enter_command("wc -l f1.txt", shell_one_line_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["1"]


def test_wc_l_two_lines(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("wc -l f1.txt", shell_sed)
    assert CommandResult.stderr == []
...
def test_wc_l_blank_lines(cl, shell_blank_lines: ShellState):
    CommandResult = cl.enter_command("wc -l blank.txt", shell_blank_lines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]


def test_wc_l_only_newlines(cl, shell_only_newlines: ShellState):
    CommandResult = cl.enter_command("wc -l nl.txt", shell_only_newlines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["4"]


def test_wc_l_whitespace_lines_count(cl, shell_whitespace_lines: ShellState):
    CommandResult = cl.enter_command("wc -l white.txt", shell_whitespace_lines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]
...
def test_wc_l_pipe_cat(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | wc -l", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["2"]


def test_wc_l_pipe_empty(cl, shell_empty: ShellState):
    CommandResult = cl.enter_command("ls | wc -l", shell_empty)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_l_pipe_grep(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | grep cat | wc -l", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["2"]


def test_wc_l_pipe_filtered_single(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | grep wolf | wc -l", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["1"]


def test_wc_l_pipe_no_matches(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | grep zebra | wc -l", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]
...
def test_wc_l_two_files(cl, shell_twofiles: ShellState):
    CommandResult = cl.enter_command("wc -l f1.txt f2.txt", shell_twofiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["2 f1.txt", "3 f2.txt", "5 total"]


def test_wc_l_multiple_files_one_empty(cl, shell_mixfiles: ShellState):
    CommandResult = cl.enter_command("wc -l a.txt b.txt", shell_mixfiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0 a.txt", "4 b.txt", "4 total"]
...
def test_wc_l_file_not_found(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -l fake.txt", shell_basic)
    assert CommandResult.stdout == []
    assert len(CommandResult.stderr) > 0


def test_wc_l_directory_error(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -l d1", shell_basic)
    assert CommandResult.stdout == []
    assert CommandResult.stderr == ["wc: cannot perform operation on directory (d1)"]


def test_wc_l_no_arguments(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -l", shell_basic)
    # depends whether your shell reads stdin or errors
    assert CommandResult.stderr == [] or len(CommandResult.stderr) > 0
...
def test_wc_l_large_file(cl, shell_largefile: ShellState):
    CommandResult = cl.enter_command("wc -l huge.txt", shell_largefile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["1000"]


def test_wc_l_trailing_newline_matters(cl, shell_trailing_newline: ShellState):
    CommandResult = cl.enter_command("wc -l t.txt", shell_trailing_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["2"]


def test_wc_l_no_final_newline(cl, shell_no_final_newline: ShellState):
    CommandResult = cl.enter_command("wc -l t.txt", shell_no_final_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["1"]
...
def test_wc_c_empty_file(cl, shell_emptyfile: ShellState):
    CommandResult = cl.enter_command("wc -c empty.txt", shell_emptyfile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_c_single_line_no_newline(cl, shell_one_line_no_newline: ShellState):
    CommandResult = cl.enter_command("wc -c f1.txt", shell_one_line_no_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["5"]


def test_wc_c_single_line_with_newline(cl, shell_one_line_newline: ShellState):
    CommandResult = cl.enter_command("wc -c f1.txt", shell_one_line_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["6"]


def test_wc_c_blank_lines(cl, shell_blank_lines: ShellState):
    CommandResult = cl.enter_command("wc -c blank.txt", shell_blank_lines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]


def test_wc_c_only_newlines(cl, shell_only_newlines: ShellState):
    CommandResult = cl.enter_command("wc -c nl.txt", shell_only_newlines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["4"]
...
def test_wc_c_whitespace_lines(cl, shell_whitespace_lines: ShellState):
    # "   "\n + "\t"\n + "hello"\n = 4 + 2 + 6 = 12
    CommandResult = cl.enter_command("wc -c white.txt", shell_whitespace_lines)
    assert CommandResult.stderr == []
...
def test_wc_c_trailing_newline(cl, shell_trailing_newline: ShellState):
    # "a"\n + "b"\n = 2 + 2 = 4
    CommandResult = cl.enter_command("wc -c t.txt", shell_trailing_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["4"]
...
def test_wc_c_no_final_newline(cl, shell_no_final_newline: ShellState):
    # "a"\n + "b" (no trailing \n) = 2 + 1 = 3
    CommandResult = cl.enter_command("wc -c t.txt", shell_no_final_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]
...
def test_wc_c_pipe_no_match(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | grep zebra | wc -c", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_c_pipe_empty_dir(cl, shell_empty: ShellState):
    CommandResult = cl.enter_command("ls | wc -c", shell_empty)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]
...
def test_wc_c_two_files(cl, shell_twofiles: ShellState):
    ...
    CommandResult = cl.enter_command("wc -c f1.txt f2.txt", shell_twofiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == [
        "4 f1.txt",
        "6 f2.txt",
...
def test_wc_c_multiple_files_one_empty(cl, shell_mixfiles: ShellState):
    ...
    CommandResult = cl.enter_command("wc -c a.txt b.txt", shell_mixfiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == [
        "0 a.txt",
        "8 b.txt",
...
def test_wc_c_file_not_found(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -c fake.txt", shell_basic)
    assert CommandResult.stdout == []
    assert len(CommandResult.stderr) > 0


def test_wc_c_directory_error(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -c d1", shell_basic)
    assert CommandResult.stdout == []
    assert CommandResult.stderr == ["wc: cannot perform operation on directory (d1)"]
...
def test_wc_c_large_file(cl, shell_largefile: ShellState):
    # "0"\n=2, "1"–"9"\n=9×2=18, "10"–"99"\n=90×3=270, "100"–"999"\n=900×4=3600 → 3890
    CommandResult = cl.enter_command("wc -c huge.txt", shell_largefile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3890"]
...
def test_wc_m_empty_file(cl, shell_emptyfile: ShellState):
    CommandResult = cl.enter_command("wc -m empty.txt", shell_emptyfile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_m_single_line_no_newline(cl, shell_one_line_no_newline: ShellState):
    CommandResult = cl.enter_command("wc -m f1.txt", shell_one_line_no_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["5"]


def test_wc_m_single_line_with_newline(cl, shell_one_line_newline: ShellState):
    CommandResult = cl.enter_command("wc -m f1.txt", shell_one_line_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["6"]


def test_wc_m_blank_lines(cl, shell_blank_lines: ShellState):
    CommandResult = cl.enter_command("wc -m blank.txt", shell_blank_lines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]


def test_wc_m_only_newlines(cl, shell_only_newlines: ShellState):
    CommandResult = cl.enter_command("wc -m nl.txt", shell_only_newlines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["4"]


def test_wc_m_whitespace_lines(cl, shell_whitespace_lines: ShellState):
    CommandResult = cl.enter_command("wc -m white.txt", shell_whitespace_lines)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["12"]


def test_wc_m_trailing_newline(cl, shell_trailing_newline: ShellState):
    CommandResult = cl.enter_command("wc -m t.txt", shell_trailing_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["4"]


def test_wc_m_no_final_newline(cl, shell_no_final_newline: ShellState):
    CommandResult = cl.enter_command("wc -m t.txt", shell_no_final_newline)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3"]
...
def test_wc_m_pipe_no_match(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("cat f1.txt | grep zebra | wc -m", shell_sed)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]


def test_wc_m_pipe_empty_dir(cl, shell_empty: ShellState):
    CommandResult = cl.enter_command("ls | wc -m", shell_empty)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["0"]
...
def test_wc_m_two_files(cl, shell_twofiles: ShellState):
    CommandResult = cl.enter_command("wc -m f1.txt f2.txt", shell_twofiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == [
        "4 f1.txt",
        "6 f2.txt",
...
def test_wc_m_multiple_files_one_empty(cl, shell_mixfiles: ShellState):
    CommandResult = cl.enter_command("wc -m a.txt b.txt", shell_mixfiles)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == [
        "0 a.txt",
        "8 b.txt",
...
def test_wc_m_file_not_found(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -m fake.txt", shell_basic)
    assert CommandResult.stdout == []
    assert len(CommandResult.stderr) > 0


def test_wc_m_directory_error(cl, shell_basic: ShellState):
    CommandResult = cl.enter_command("wc -m d1", shell_basic)
    assert CommandResult.stdout == []
    assert CommandResult.stderr == ["wc: cannot perform operation on directory (d1)"]


def test_wc_m_large_file(cl, shell_largefile: ShellState):
    CommandResult = cl.enter_command("wc -m huge.txt", shell_largefile)
    assert CommandResult.stderr == []
    assert CommandResult.stdout == ["3890"]
...
def test_wc_lm_two_lines(cl, shell_sed: ShellState):
    CommandResult = cl.enter_command("wc -l -m f1.txt", shell_sed)
    assert CommandResult.stderr == []
    parts = CommandResult.stdout[0].split()
    assert parts[0] == "2"
    assert parts[1] == "20"
...
def test_wc_lmc_ascii_chars_equal_bytes(cl, shell_sed: ShellState):
    ...
    CommandResult = cl.enter_command("wc -l -m -c f1.txt", shell_sed)
    assert CommandResult.stderr == []
    parts = CommandResult.stdout[0].split()
