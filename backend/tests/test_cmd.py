...
def test_mkdir_permissions(cl, shell_empty: ShellState):
    ...
    CmdResult = cl.enter_command(f'mkdir -m a=00 a', shell_empty)
    assert CmdResult.stderr == ['chmod: value given for permissions which is not of length of 3']
    assert CmdResult.stdout == []

    CmdResult = cl.enter_command(f'mkdir -m randomstuff a', shell_empty)
...
def test_chmod_basic(cl, shell_basic: ShellState):
    ...
    assert CmdResult.stdout == ["Updated permissions of d1 with d---------", 'Updated permissions of f3.txt with ----------', 'Updated permissions of f4.txt with ----------']
    assert CmdResult.stderr == []
...
def test_chmod_errors(cl, shell_basic: ShellState):
    ...
    assert CmdResult.stderr == ["chmod: value other then given integer given for permissions"]
    assert CmdResult.stdout == []
...
def test_vm_move_multiple(cl, shell_basic: ShellState):
    ...
    assert CmdResult.stderr == ["mv: could not find file f5.txt", "mv: could not find file f6.txt"]
    assert CmdResult.stdout == []
...
def test_pipes_head_tail_combo(cl, shell_fouritems: ShellState):
    CmdResult = cl.enter_command('ls | head --lines=3 | tail --lines=1', shell_fouritems)
    assert CmdResult.stderr == []
...
def test_cp_errors(cl, shell_cp: ShellState):
    ...
    assert CmdResult.stdout == []
    assert CmdResult.stderr == ["cp: cannot overwrite non-directory 'f1.txt' with directory 'project'"]
