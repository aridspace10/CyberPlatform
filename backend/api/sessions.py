...
from services.session_service import get_session_shell, wipe_session_db, delete_session_shell, get_session, get_sandbox_session, add_session, get_scenario_byname, add_session_scenario, add_session_shell, update_session_shell
...
def clear_session(session_id: int, db: Session = Depends(get_db)):
    ...
    ses = session_manager.get_session(str(session_id))
    if ses != "404":
        session_manager.remove_session(str(session_id))

    # Remove DB records
    delete_session_shell(db, session_id)
...
def clear_all_sessions(db: Session = Depends(get_db)):
    # Clear all from session manager
