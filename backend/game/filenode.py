...
from game.Parser import AndNode, ExecNode, FilterNode, Node, NotNode, OrNode
...
class FileNode:
    ...
    def _evalOrFindNode(
        self, node: OrNode, actions: list[str]
    ) -> Tuple[bool, list[str]]:
    ...
    def list_content(
        ...
        for item in items:
            ...
            if not detail:
                ...
                tmp.extend(
                    [
                        self.get_permission_str(item),
                        str(1),
                        "user",
                        "user",
                        str(item.get_size()),
                        str(self.inode.mtime.strftime("%b")),
